...
export const Filters = () => {
	...
	const issStateEmpty =
		state && Object.keys(state).filter(k => state[k]).length > 0 ? false : true

	const minPriceRef = useRef<HTMLInputElement>(null)
	const maxPriceRef = useRef<HTMLInputElement>(null)

	const handlePriceRange = () => {
		const minP = Number(minPriceRef?.current?.value)
		const maxP = Number(maxPriceRef?.current?.value)
			...
			<ul className='filters d-flex p-relative fs-400'>
				...
				<li className='price-range p-relative'>
					...
					<div className='slider-input'>
						<span>${state?.minPriceVal}  </span>-
						<span>  ${state?.maxPriceVal}</span>
					</div>
